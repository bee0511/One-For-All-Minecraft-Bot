...
let logger,
  ...
  bot,
  csafe_success = false;
let mapartBuildUseTime = 0;
let cartography_t_vec3;
let cartography_t_s_vec3;
// ????????(????角度 ?工??)
let mp_direction = {
  north: {
    //2b
...
let mapart_cfg = {
  ...
  wrap: {
    ...
    facing: "north",
    name: "ExampleMP_Name",
    source: "https://www.pixiv.net/artworks/92433849", //???? ??完全????
...
let mapart_global_cfg = {
  ...
  discord_webhookURL:
    "https://discord.com/api/webhooks/1234567890123456789/abc",
  replaceMaterials: [],
};
/** @type {OFABot.CommandGroup} */
const mapart = {
  identifiers: ["mapart", "mp", "map"],
  commands: [
    {
      //test
      name: "test",
  ...
  async init(bott, user_id, lg) {
    ...
    if (!fs.existsSync(`${process.cwd()}/config/${bot_id}/mapart.json`)) {
      ...
      //bot.logger(true,"INFO",process.argv[2],`????Bot???設???..`)
      try {
        mapart_cfg = await readConfig(
          `${process.cwd()}/config/${bot_id}/mapart.json`,
        );
      } catch (e) {
        bot.logger(
          true,
        ...
        console.log(
          `\x1b[33mhttps://www.google.com/search?q=${e.message.replaceAll(" ", "+")}\x1b[0m`,
        );
        bot.gkill(202);
      }
    }
    ...
    if (!fs.existsSync(`${process.cwd()}/config/global/mapart.json`)) {
      logger(
        ...
        "INFO",
        process.argv[2],
        `Creating global config - mapart.json`,
      );
		await fsp.writeFile(
			`${process.cwd()}/config/global/mapart.json`,
			JSON.stringify(mapart_global_cfg, null, "\t"),
		);
    } else {
      try {
        mapart_global_cfg = await readConfig(
          `${process.cwd()}/config/global/mapart.json`,
        );
      } catch (e) {
        bot.logger(
...
async function mp_set(task) {
  let mapart_set_cache = await readConfig(
    `${process.cwd()}/config/${bot_id}/mapart.json`,
  );
  if (!fs.existsSync(mapart_global_cfg.schematic_folder + task.content[2])) {
    await taskreply(
      task,
      `&7[&bMP&7] &c?????&7${task.content[2]} &r???`,
      `????????\n?? ${mapart_global_cfg.schematic_folder}\n?: ${task.content[2]}`,
      null,
    );
    return;
  }
  ...
  if (
    Math.abs(mapart_set_cache.schematic.placementPoint_x + 64) % 128 != 0 &&
    task.content[6] != "-f"
  ) {
    await taskreply(task, `&7[&bMP&7] &cX?????`, `X?????`, null);
    return;
  }
  try {
    await fsp.writeFile(
      `${process.cwd()}/config/${bot_id}/mapart.json`,
      JSON.stringify(mapart_set_cache, null, "\t"),
    );
  } catch (e) {
    await taskreply(task, `&7[&bMP&7] &c設置?`, `設置? ${e}`, null);
    return;
  }
  mapart_cfg = mapart_set_cache;
  await taskreply(task, `&7[&bMP&7] &a設置??`, `設置??`, null);
}
async function mp_info(task) {
  let mapart_info_cfg_cache = await readConfig(
    `${process.cwd()}/config/${bot_id}/mapart.json`,
  );
  ...
  switch (task.source) {
    case "minecraft-dm":
      bot.chat(
        `/m ${task.minecraftUser} ${mapart_info_cfg_cache.schematic.filename} ${prog}%`,
      );
      break;
    case "console":
...
async function mp_build(task) {
  let mapart_build_cfg_cache = await readConfig(
    `${process.cwd()}/config/${bot_id}/mapart.json`,
  );
  ...
  if (mapart_build_cfg_cache.server == -1) {
    console.log(`&7${mapart_build_cfg_cache.server} TAB ??讀????試`);
    return;
  }
  ...
  //try {
  await litematicPrinter.build_file(
    task,
    bot,
    litematicPrinter.model_mapart,
    mapart_build_cfg_cache,
  );
  ...
  mapartBuildUseTime =
    (build_result_query.endTime - build_result_query.startTime) / 1000;
  console.log(
    `消???${Math.floor(mapartBuildUseTime / 3600)} h ${Math.floor((mapartBuildUseTime % 3600) / 60)} m ${Math.floor(mapartBuildUseTime % 60)} s`,
  );
  ...
  const webhookClient = new WebhookClient({
    url: mapart_global_cfg.discord_webhookURL,
  });
  if (crt_filename_match) {
    crtFileIndex = [
      parseInt(crt_filename_match[1]),
      parseInt(crt_filename_match[2]),
    ];
  }
  ...
  if (FLAG_autonext) {
    //check args to build next ?
    let autonext_stopAt;
    if (FLAG_autonext_value) {
      const autonext_match = FLAG_autonext_value.match(auto_regex);
      autonext_stopAt = [
        parseInt(autonext_match[1]),
        parseInt(autonext_match[2]),
      ];
    }
    //console.log(autonext_stopAt)
    let matchStop = false; //?? ? ??//!FLAG_autonext_value
    if (!crtFileIndex) matchStop = true;
    let nextFileName;
    if (
      !matchStop &&
      FLAG_autonext_value &&
      autonext_stopAt[0] == crtFileIndex[0] &&
      autonext_stopAt[1] == crtFileIndex[1]
    )
      matchStop = true; //?
    if (!matchStop) {
      //get next index  ??
      ...
      if (
        fs.existsSync(
          `${mapart_build_cfg_cache.schematic.folder}${crt_filename}_${crtFileIndex[0]}_${crtFileIndex[1] + 1}.${crt_filename_type}`,
        )
      ) {
        nextFileName = `${crt_filename}_${crtFileIndex[0]}_${crtFileIndex[1] + 1}.${crt_filename_type}`;
      } else if (
        fs.existsSync(
          `${mapart_build_cfg_cache.schematic.folder}${crt_filename}_${crtFileIndex[0] + 1}_0.${crt_filename_type}`,
        )
      ) {
  ...
  function gen_mapartAutoFinishEmbed() {
    ...
    let mapartfinishEmbed = {
      ...
      fields: [
        ...
        {
          name: "???",
          value: `<t:${Math.floor(build_result_query.endTime / 1000)}:f>`,
          inline: false,
        },
      ],
    };
  ...
  function gen_mapartFinishEmbed() {
    ...
    let mapartfinishEmbed = {
      color: 0x0099ff,
      title: `${mapart_build_cfg_cache.schematic.filename} build finished`,
      //url: 'https://discord.js.org',
      author: {
        name: bot.username,
        icon_url: iconurl,
      ...
      fields: [
        ...
        {
          name: "????",
          value: `<t:${Math.floor(build_result_query.startTime / 1000)}:f>`,
          inline: true,
        },
        {
          name: "???",
        ...
        {
          name: "elapsed",
          value: `${Math.floor(mapartBuildUseTime / 3600)} h ${Math.floor((mapartBuildUseTime % 3600) / 60)} m ${Math.floor(mapartBuildUseTime % 60)} s`,
          inline: true,
        },
        {
          name: "Speed",
...
async function mp_test(task) {
  ...
  let stationConfig = await readConfig(
    `${process.cwd()}/config/global/${mapart_build_cfg_cache.station}`,
  );
  let needReStock = [
...
async function mp_open(task) {
  ...
  let mapart_open_cfg_cache = await readConfig(
    `${process.cwd()}/config/${bot_id}/mapart.json`,
  );
  let stationConfig;
  if (mapart_open_cfg_cache["materialsMode"] == "station") {
    stationConfig = await readConfig(
      `${process.cwd()}/config/global/${mapart_open_cfg_cache.station}`,
    );
  }
  ...
  const sx =
    Math.floor((Math.floor(bot.entity.position.x / 16) - 4) / 8) * 8 * 16 + 64;
  const sz =
    Math.floor((Math.floor(bot.entity.position.z / 16) - 4) / 8) * 8 * 16 + 64;
  const mapart_ori = new Vec3(
    sx + 1,
    mapart_open_cfg_cache["schematic"]["placementPoint_y"] - 2,
    sz,
  );
  console.log(mapart_ori);
  await pathfinder.astarfly(
    bot,
    mapart_ori.offset(0, 0, 3),
    null,
    null,
    null,
    false,
  ...
  for (let dx = 0; dx < mapart_open_cfg_cache["open"]["width"]; dx++) {
    for (let dy = 0; dy < mapart_open_cfg_cache["open"]["height"]; dy++) {
      let csmp = {
        ...
        mapartRealPos: new Vec3(
          sx + 128 * (dx * mapart_open_cfg_cache["open"]["height"] + dy),
          256,
          sz,
        ),
      ...
      if (
        currentIF?.metadata &&
        currentIF?.metadata[8]?.itemId == mcData.itemsByName["filled_map"].id
      ) {
  ...
  for (let i = 0; i < mpstate.length; ) {
    ...
    if (bot.blockAt(mpstate[i].pos.offset(0, 0, -1)).name == "air") {
      if (
        ...
        bot.inventory?.slots[44].name != blockToAdd
      ) {
        ...
        if (invhasblockToAdd == -1) {
          if (mapart_open_cfg_cache["materialsMode"] == "station") {
            await sleep(5000);
            await stationRestock(stationConfig, [
              { name: blockToAdd, count: 64 },
            ]);
            await sleep(500);
            await mcFallout.warp(bot, mapart_open_cfg_cache["open"]["warp"]);
            await sleep(2500);
  ...
  for (let i = 0; i < mpstate.length; ) {
    ...
    if (
      !bot.inventory?.slots[44] ||
      bot.inventory?.slots[44].name != "glow_item_frame"
    ) {
      let invhasblockToAdd = -1;
      for (let id = 9; id <= 43; id++) {
        //if (bot.inventory.slots[id]) console.log(bot.inventory.slots[id].name)
    ...
    bot.activateBlock(
      bot.blockAt(mpstate[i].pos.offset(0, 0, -1)),
      new Vec3(0, 0, 1),
    );
  ...
  async function openMap(mps) {
    ...
    //console.log(mps)
    while (
      bot.entity.position.distanceTo(mps.mapartRealPos.offset(64, 0, 64)) > 20
    ) {
      await pathfinder.astarfly(
        bot,
        mps.mapartRealPos.offset(64, 0, 64),
        null,
        null,
        null,
        true,
    ...
    for (let off_x = 0; off_x < 128; off_x += offset) {
      await pathfinder.astarfly(
        bot,
        mps.mapartRealPos.offset(off_x, 0, 64),
        null,
        null,
        null,
        true,
    ...
    mps.mapid = mpID;
    await moveToEmptySlot(44);
    console.log(`mp_${mps.x}_${mps.y} ? ${mpID}`);
  }
  async function putMapON() {
    ...
    for (let inv_i = 9; inv_i < 44; inv_i++) {
      if (
        bot.inventory.slots[inv_i] &&
        bot.inventory.slots[inv_i].name == "filled_map"
      ) {
  ...
  function getItemFrame(tg_pos) {
    for (let etsIndex in bot.entities) {
      if (
        !(
          bot.entities[etsIndex].mobType == "Glow Item Frame" ||
          bot.entities[etsIndex].mobType == "Item Frame"
        )
      )
        continue;
...
async function mp_name(task) {
  const Item = require("prismarine-item")(bot.version);
  let mapart_name_cfg_cache = await readConfig(
    `${process.cwd()}/config/${bot_id}/mapart.json`,
  );
  let stationConfig;
  if (mapart_name_cfg_cache["materialsMode"] == "station") {
    stationConfig = await readConfig(
      `${process.cwd()}/config/global/${mapart_name_cfg_cache.station}`,
    );
  }
  ...
  await pathfinder.astarfly(
    bot,
    new Vec3(
      mapart_name_cfg_cache.wrap.anvil_stand[0],
      mapart_name_cfg_cache.wrap.anvil_stand[1],
      mapart_name_cfg_cache.wrap.anvil_stand[2],
    ),
  );
  let mp_origin = new Vec3(
    mapart_name_cfg_cache.wrap.origin[0],
    mapart_name_cfg_cache.wrap.origin[1],
    mapart_name_cfg_cache.wrap.origin[2],
  );
  //console.log(abc.metadata[8].nbtData.value.display.value.Lore)
  // value: [
  //     '{"extra":[{"bold":false,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"color":"gray","text":"???顯示"}],"text":""}',
  //     '{"extra":[{"italic":false,"color":"gray","text":"?以???"},{"italic":false,"color":"dark_gray","text":"(作者可使用 /copyright 變更)"}],"text":""}',
  //     '{"text":""}',
  ...
  //     '{"text":""}',
  //     '{"extra":[{"bold":false,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"color":"dark_gray","text":"?識別?: "}],"text":""}',
  ...
  //     '{"extra":[{"bold":false,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"color":"dark_gray","text":"????作?願????使用??}],"text":""}',
  //     '{"extra":[{"bold":false,"italic":false,"underlined":false,"strikethrough":false,"obfuscated":false,"color":"dark_gray","text":"????侵???????}],"text":""}'
  //   ]
  let maparts = [];
  ...
  for (
    let i = 0;
    i <
    mapart_name_cfg_cache["wrap"]["width"] *
      mapart_name_cfg_cache["wrap"]["height"];
    i++
  ) {
    let d_x = Math.floor(i / mapart_name_cfg_cache["wrap"]["height"]);
    ...
    let mps = {
      dx: d_x,
      dy: d_y,
      pos: mp_origin.offset(
        d_x * mp_direction[facing]["inc_dx"],
        d_y * mp_direction[facing]["inc_dy"],
        d_x * mp_direction[facing]["inc_dz"],
      ),
      hasmap: false,
      mapid: undefined,
      named: false,
    ...
    if (mps.hasmap) {
      mps.named = currentIF.metadata[8].nbtData.value.display.value.Name
        ? true
        : false;
    }
    //console.log(mps.pos)
  ...
  for (
    ...
    if (!itemFrame) {
      console.log(mps, "item frame missing");
      continue;
    }
    console.log(`?? ${mps.dx}_${mps.dy}`);
    ...
    try {
      ...
      console.log("pick map item failed", mps);
    }
    await pathfinder.astarfly(
      bot,
      new Vec3(
    ...
    await sleep(50);
    let anvil = await bot.openAnvil(
      bot.blockAt(
        new Vec3(
          mapart_name_cfg_cache.wrap.anvil[0],
          mapart_name_cfg_cache.wrap.anvil[1],
          mapart_name_cfg_cache.wrap.anvil[2],
        ),
      ),
    );
    let it = getMapItemByMapIDInInventory(mps.mapid);
    //console.log(it)
    let tgname = mapart_name_cfg_cache["wrap"]["name"]
      ? `${mapart_name_cfg_cache["wrap"]["name"]} &r- &b${mps.dx}-${mps.dy}`
      : `&b${mps.dx}-${mps.dy}`;
    console.log(`?? ${mps.dx}_${mps.dy}`);
    ...
    while (fail_c < 10) {
      ...
      //console.log("??置\n",bot.inventory.slots[44])
      console.log(`??mp_${mps.dx}_${mps.dy}`);
      await bot.activateEntity(getItemFrame(mps.pos));
      await sleep(1000);
      let check = getItemFrame(mps.pos);
    ...
    console.log(`${mps.dx}_${mps.dy} \x1b[32m?\x1b[0m`);
    //break;
  }
  function getItemFrame(tg_pos) {
    for (let etsIndex in bot.entities) {
...
async function mp_material(task) {
  ...
  if (task.content[4] - task.content[7] === 0) {
    ...
    for (z; z >= +task.content[9]; z--) {
      for (let i = y - 5; i >= -64; i--) {
        ...
        if (["air", "cave_air", "bedrock"].includes(block.name)) {
          break;
        }
  ...
  await taskreply(task, `&7[&bMP&7] &c????&r使用mapart file導出`, null);
  return;
}
async function mp_file(task) {
  ...
  await taskreply(
    task,
    `&7[&bMP&7] &c已輸???{task.content[2]}.json &r請確認Bot???`,
    null,
  );
  return;
}
async function mp_copy(task) {
  ...
  let mp_shu_origin = new Vec3(
    mapart_name_cfg_cache.wrap.copy_f_shulker[0],
    mapart_name_cfg_cache.wrap.copy_f_shulker[1],
    mapart_name_cfg_cache.wrap.copy_f_shulker[2],
  );
  let maparts = [];
  let facing = mapart_name_cfg_cache["wrap"]["facing"];
  cartography_t_vec3 = v(mapart_name_cfg_cache["wrap"]["cartography_table"]);
  cartography_t_s_vec3 = v(
    mapart_name_cfg_cache["wrap"]["cartography_table_stand"],
  );
  let standOffest = new Vec3(
    mp_direction[facing]["inc_dx"],
    mp_direction[facing]["inc_dy"],
    mp_direction[facing]["inc_dz"],
  ...
  for (
    ...
    let mps = {
      ...
      box: mp_shu_origin.offset(
        boxoffset * mp_direction[facing]["inc_dx"],
        0,
        boxoffset * mp_direction[facing]["inc_dz"],
      ),
      s: i % 27, //slot
      hasmap: false,
      mapid: undefined,
  ...
  for (let i = 0; i < box_amount; i++) {
    console.log(`檢查?${i + 1}??子`);
    //console.log(mp_shu_origin)
    let boxVec = mp_shu_origin.offset(
      mp_direction[facing]["inc_dx"] * i,
      0,
      mp_direction[facing]["inc_dz"] * i,
    );
    //console.log("pos",boxVec.offset(standOffest.x,standOffest.y,standOffest.z))
    await pathfinder.astarfly(
      bot,
      boxVec.offset(standOffest.x, standOffest.y, standOffest.z),
      null,
      null,
      null,
      true,
    ...
    while (t++ < 3 && !shulker_box) {
      shulker_box = await containerOperation.openContainerWithTimeout(
        bot,
        boxVec,
        1000,
      );
    ...
    if (!shulker_box) {
      console.log(`????-${i + 1} ?`, boxVec);
      return;
    }
    for (
      let shu_index = 0;
      shu_index < 27 && checkIndex < maparts.length;
      shu_index++, checkIndex++
    ) {
      if (shulker_box.slots[shu_index] == null) continue;
      if (shulker_box.slots[shu_index].name != "filled_map") {
        console.log(
          `box-${i + 1}-${shu_index}丟出???${shulker_box.slots[shu_index].name}`,
        );
        await bot.simpleClick.leftMouse(shu_index);
        await bot.simpleClick.leftMouse(-999);
      } else if (
        shulker_box.slots[shu_index]?.nbt?.value?.map?.value !=
        maparts[checkIndex].mapid
      ) {
        ...
        console.log(`box-${i + 1}-${shu_index} map-id?常`);
  ...
  for (
    ...
    let mps = maparts[i];
    if (!mps.hasmap || mps.amount >= mapart_name_cfg_cache["wrap"].copy_amount)
      continue;
    if (
      bot.inventory.count(mcData.itemsByName["map"].id) <
      mapart_name_cfg_cache["wrap"].copy_amount - mps.amount
    ) {
      ...
      for (
        let require_amount_iterator = i;
        require_amount_iterator < maparts.length;
        require_amount_iterator++
      ) {
      ...
      if (mapart_name_cfg_cache["materialsMode"] == "station") {
        ...
        await stationRestock(stationConfig, [
          {
            name: "map",
            count: require_amount > cap ? cap : require_amount,
          },
        ]);
        await sleep(500);
        //await stationRestock(stationConfig, [{ name: "map", count: mpstate.length }])
    ...
    while (!shulker_box && mt++ < 3) {
      await pathfinder.astarfly(
        bot,
        mps.box.offset(standOffest.x, standOffest.y, standOffest.z),
        null,
        null,
        null,
        true,
    ...
    if (mapart_name_cfg_cache["wrap"].copy_amount - mps.amount == 64) {
      ...
      for (let ff = 27; ff <= 62; ff++) {
        if (
          shulker_box.slots[ff]?.nbt?.value?.map?.value == mps.mapid &&
          shulker_box.slots[ff]?.count == 64
        ) {
          tgmp = ff;
          break;
        }
...
async function mp_wrap(task) {
   ...
   * check input shulker
   */
  let input = await containerOperation.openContainerWithTimeout(
    bot,
    inputVec,
    500,
  );
  ...
  for (let i = 0; i < counter; i++) {
    let input2,
      t = 0;
    while (t++ < 3 && !input2) {
      input2 = await containerOperation.openContainerWithTimeout(
        bot,
        inputVec,
        1000,
      );
    ...
    for (let gg = 0; gg < 27; gg++) {
      ...
      let emptySlot = input2.firstEmptySlotRange(
        input2.inventoryStart,
        input2.inventoryEnd,
      );
    ...
    let output,
      t2 = 0;
    while (t2++ < 10 && !output) {
      output = await containerOperation.openContainerWithTimeout(
        bot,
        outputVec,
        1000,
      );
    ...
    for (let gg = 0; gg < 27; gg++) {
      ...
      if (wrap_items[gg].name == "filled_map") {
        ...
        for (let ff = 27; ff <= 62; ff++) {
          if (
            output.slots[ff]?.nbt?.value?.map?.value == wrap_items[gg].mapid
          ) {
            tgmp = ff;
            break;
          }
...
function getMapItemByMapIDInInventory(mpID) {
  ...
  // }
  return bot.inventory.slots.find(
    (item) => item?.nbt?.value?.map?.value == mpID,
  );
}
async function mapCopy(cartography_table_pos, mapid, amount = 1) {
  ...
  let ttry = 0;
  while (!ct && ttry < 3) {
    ct = await containerOperation.openContainerWithTimeout(
      bot,
      cartography_table_pos,
      500,
    );
  }
  ...
  async function mapCopyOne(cartography_table, mapid) {
    let targetMap = cartography_table.slots.find(
      (item) => item?.nbt?.value?.map?.value == mapid,
    );
...
async function pickMapItem(mpID) {
  ...
  while (true) {
    ...
    for (let i in et) {
      if (
        et[i]?.mobType == "Item" &&
        et[i]?.metadata[8]?.itemId == mcData.itemsByName["filled_map"].id &&
        et[i]?.metadata[8].nbtData?.value?.map?.value == mpID
      ) {
        if (et[i].onGround)
          await pathfinder.astarfly(
            bot,
            new Vec3(
              Math.round(et[i].position.x - 0.5),
              Math.round(et[i].position.y - 1),
              Math.round(et[i].position.z - 0.5),
            ),
            null,
            null,
            null,
  ...
  if (timeout) throw new Error("pick map item timeout");
  try {
    clearTimeout(to);
  } catch (e) {}
}
...
async function stationRestock(stationConfig, RS_obj_array) {
  ...
  for (let index = 0; index < RS_obj_array.length; index++) {
    await st_restock_single(
      stationConfig,
      RS_obj_array[index].name,
      RS_obj_array[index].count,
    );
  }
  async function st_restock_single(stationConfig, restockid, quantity) {
    ...
    let botton_loc;
    let standPos;
    let stand_dirc_offset =
      stationConfig.offset[
        stationConfig.materials[findItemMaterialsIndex][1][3]
      ];
    let botton_dirc_offset =
      stationConfig.offset[
        stationConfig.materials[findItemMaterialsIndex][1][4]
      ];
    if (stand_dirc_offset == undefined || botton_dirc_offset == undefined) {
      if (
        bot
          .blockAt(shulkerBox_loc.offset(-1, 0, 0))
          .name.indexOf("comparator") != -1
      ) {
        ...
        bot
          .blockAt(shulkerBox_loc.offset(0, 0, -1))
          .name.indexOf("comparator") != -1
      ) {
      ...
      standPos = shulkerBox_loc.offset(
        stand_dirc_offset[0],
        stand_dirc_offset[1],
        stand_dirc_offset[2],
      );
      botton_loc = shulkerBox_loc.offset(
        botton_dirc_offset[0],
        botton_dirc_offset[1],
        botton_dirc_offset[2],
      );
    }
    if (standPos.distanceTo(bot.entity.position) > 100) {
      console.log("distance too far, warp to station");
      console.log(`??中 ${stationConfig.stationWarp}`);
    ...
    if (quantity == -1) {
      ...
      while (maxTryTime++ < 5) {
        ...
        try {
          shu = await pTimeout(
            bot.openBlock(bot.blockAt(shulkerBox_loc)),
            1000,
          );
          success_open = true;
      ...
      if (success_open) {
        remain = await containerOperation.deposit(
          bot,
          shu,
          restockid,
          -1,
      ...
      if (remain > 0 || remain == -1) {
        ...
        let ofstand_dirc_offset =
          stationConfig.offset[
            stationConfig.materials[findItemMaterialsIndex][1][3]
          ];
        let ofbotton_dirc_offset =
          stationConfig.offset[
            stationConfig.materials[findItemMaterialsIndex][1][4]
          ];
        if (
          ofstand_dirc_offset == undefined ||
          ofbotton_dirc_offset == undefined
        ) {
          if (
            bot
              .blockAt(overfull_shu_loc.offset(-1, 0, 0))
              .name.indexOf("comparator") != -1
          ) {
            ...
            bot
              .blockAt(overfull_shu_loc.offset(0, 0, -1))
              .name.indexOf("comparator") != -1
          ) {
          ...
          overfull_standPos = shulkerBox_loc.offset(
            ofstand_dirc_offset[0],
            ofstand_dirc_offset[1],
            ofstand_dirc_offset[2],
          );
          overfull_botton_loc = shulkerBox_loc.offset(
            ofbotton_dirc_offset[0],
            ofbotton_dirc_offset[1],
            ofbotton_dirc_offset[2],
          );
        }
        await pathfinder.astarfly(
          bot,
        ...
        while (maxTryTime++ < 5) {
          try {
            shu = await pTimeout(
              bot.openBlock(bot.blockAt(overfull_shu_loc)),
              1000,
            );
            success_open = true;
            console.log("container opened");
      ...
      ii: while (maxTryTime++ < 10) {
        ...
        if (success_open) {
          console.log("???..");
          let tempremain = await containerOperation.withdraw(
            bot,
            shu,
            restockid,
            remain,
...
async function readConfig(file) {
	...
	var com_file = await JSON.parse(raw_file.toString());
	return com_file;
}
