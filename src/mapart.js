...
async function mp_set(task) {
	...
	try {
		...
		await taskreply(task, `&7[&bMP&7] &c設置失敗`, `設置失敗 ${e}`, null);
		return;
	}
	mapart_cfg = mapart_set_cache;
	await taskreply(task, `&7[&bMP&7] &a設置成功`, `設置成功`, null);
}
...
async function mp_build(task) {
	...
	mapart_build_cfg_cache.schematic.folder =
		mapart_global_cfg.schematic_folder;
	mapart_build_cfg_cache.bot_id = bot_id;
	mapart_build_cfg_cache.replaceMaterials =
		mapart_global_cfg.replaceMaterials;
	...
	if (mapart_build_cfg_cache.server == -1) {
		console.log(
			`&7${mapart_build_cfg_cache.server} TAB 分流讀取失敗 請重試`,
		);
		return;
	}
	...
	console.log(
		`消耗時間 ${parseInt(mapartBuildUseTime / 3600)} h ${parseInt((mapartBuildUseTime % 3600) / 60)} m ${parseInt(mapartBuildUseTime % 60)} s`,
	);
...
async function mp_open(task) {
	...
	const sx =
		Math.floor((Math.floor(bot.entity.position.x / 16) - 4) / 8) * 8 * 16 +
		64;
	const sz =
		Math.floor((Math.floor(bot.entity.position.z / 16) - 4) / 8) * 8 * 16 +
		64;
		...
		for (let dy = 0; dy < mapart_open_cfg_cache["open"]["height"]; dy++) {
			let csmp = {
				...
				mapartRealPos: new Vec3(
					sx +
						128 *
							(dx * mapart_open_cfg_cache["open"]["height"] + dy),
					256,
					sz,
				),
			...
			if (
				currentIF?.metadata &&
				currentIF?.metadata[8]?.itemId ==
					mcData.itemsByName["filled_map"].id
			) {
			...
			if (
				...
				if (invhasblockToAdd == -1) {
					if (mapart_open_cfg_cache["materialsMode"] == "station") {
						...
						await sleep(500);
						await mcFallout.warp(
							bot,
							mapart_open_cfg_cache["open"]["warp"],
						);
						await sleep(2500);
					}
					continue;
			...
			if (
				...
				if (mps) {
					...
					while (fail_c < 10) {
						...
						if (
							check &&
							check.metadata[8].nbtData.value.map.value == mpID
						) {
							break;
						}
						fail_c++;
	...
	for (
		...
		while (fail_c < 10) {
			...
			if (!getItemFrame(mps.pos)) {
				await mcFallout.warp(
					bot,
					mapart_name_cfg_cache["wrap"]["warp"],
				);
			...
			if (
				check &&
				check.metadata[8]?.nbtData?.value?.map?.value == mps.mapid
			) {
				break;
			}
			fail_c++;
...
async function mp_copy(task) {
	...
	for (let i = 0; i < box_amount; i++) {
		...
		for (
			...
			if (shulker_box.slots[shu_index].name != "filled_map") {
				console.log(
					`box-${i + 1}-${shu_index}丟出異物 ${shulker_box.slots[shu_index].name}`,
				);
	...
	for (
		...
		if (
			!mps.hasmap ||
			mps.amount >= mapart_name_cfg_cache["wrap"].copy_amount
		)
			continue;
		if (
		...
		if (mapart_name_cfg_cache["wrap"].copy_amount - mps.amount == 64) {
			...
			for (let ff = 27; ff <= 62; ff++) {
				if (
					shulker_box.slots[ff]?.nbt?.value?.map?.value ==
						mps.mapid &&
	...
	for (let i = 0; i < counter; i++) {
		...
		for (let gg = 0; gg < 27; gg++) {
			...
			if (wrap_items[gg].name == "filled_map") {
				...
				for (let ff = 27; ff <= 62; ff++) {
					if (
						output.slots[ff]?.nbt?.value?.map?.value ==
						wrap_items[gg].mapid
					) {
						tgmp = ff;
						break;
...
async function mapCopy(cartography_table_pos, mapid, amount = 1) {
	...
	async function mapCopyOne(cartography_table, mapid) {
		...
		outputitem.slot = 2;
		outputitem.count =
			outputitem.count * 2 > 64 ? 64 : outputitem.count * 2;
		//let slot0
...
async function stationRestock(stationConfig, RS_obj_array) {
	...
	async function st_restock_single(stationConfig, restockid, quantity) {
		...
		for (
			let fIMI_i = 0;
			fIMI_i < stationConfig.materials.length;
			fIMI_i++
		) {
		...
		let shulkerBox_loc = v(
			stationConfig.materials[findItemMaterialsIndex][1],
		);
		let botton_loc;
		let standPos;
