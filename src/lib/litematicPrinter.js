...
let pause = false,
  stop = false;
let BLOCK_EXCLUDE_LIST = ["air", "cave_air"];
let BLOCK_EQUIVALENT_LIST = {
  空氣: ["air", "water", "brown_mushroom"],
  泥土: ["grass_block", "dirt", "mycelium"],
...
const litematicPrinter = {
  ...
  build_file: async function (task, bot, model, cfg) {
    if (!fs.existsSync(cfg.schematic.folder + cfg.schematic.filename)) {
      console.log(
        `&7[&LP&7] &c?�發?��?�?&7${cfg.schematic.filename} &r請檢?�設定`,
      );
      return;
    }
...
async function model_mapart_build(task, bot, cfg, project) {
  ...
  if (
    !fs.existsSync(`${process.cwd()}/config/${cfg.bot_id}/build_cache.json`)
  ) {
    await save_cache(cfg.bot_id, build_cache);
  } else {
    build_cache = await readConfig(
      `${process.cwd()}/config/${cfg.bot_id}/build_cache.json`,
    );
  }
  let stationConfig;
  let wmsConfig;
  if (cfg.materialsMode == "station") {
    bot.logger(true, "INFO", `?��??��?站�?�?..`);
    try {
      stationConfig = await readConfig(
        `${process.cwd()}/config/global/${cfg.station}`,
      );
    } catch (e) {
      bot.logger(
        true,
    ...
    bot.logger(true, "INFO", `?��?�?${stationConfig.stationName} ?��??��?`);
  }
  let targetSch;
  if (project) {
    targetSch = project;
  } else {
    targetSch = await schematic.loadFromFile(
      cfg.schematic.folder + cfg.schematic.filename,
    );
  }
  if (build_cache.hash != crt_cfg_hash || cfg["ignore_cache"]) {
    ...
    build_cache.placedBlock = 0; //?�該不太準確 ?��?要�?修改
    ...
    build_cache.destination = build_cache.origin
      .plus(targetSch.Metadata.EnclosingSize)
      .offset(-1, -1, -1);
    build_cache.placement_origin = new Vec3(
      cfg.schematic.placementPoint_x,
      cfg.schematic.placementPoint_y,
      cfg.schematic.placementPoint_z,
    );
    build_cache.placement_destination = build_cache.placement_origin.plus(
      build_cache.destination,
    );
    build_cache.debug = {
      discconnectCount: 0,
  ...
  bot.logger(true, "INFO", `?��?建�?${targetSch.Metadata.Name}`);
  targetSch.toMineflayerID();
  // replace the material here
  for (const i in cfg.replaceMaterials) {
    targetSch.changeMaterial(
      cfg.replaceMaterials[i][0],
      cfg.replaceMaterials[i][1],
    );
  }
  ...
  if (cfg.materialsMode == "station") {
    ...
    if (stationNotSupports.length != 0) {
      console.log(`?�現?�支?��??��?`, stationNotSupports);
      throw new Error(
        `Not Support Materials: ${stationNotSupports.join(", ")}`,
      );
    }
  } else if (false) {
    //wms
  ...
  for (const i in targetSch.palette) {
    ...
    if (needFirstBuildList.includes(targetSch.palette[i].Name)) {
      cobblestoneIndex.push(Number(i));
    }
  }
  let f_tmp = [];
  ...
  let Block_In_CD = []; //?? currentPaletteBlocksIndexs �?index
  let removeTimerID = [];
  let currentPaletteBlocksIndexs = []; //
  let currentPaletteBlocksState = []; // [0 ?��???][1 已�??�];
  let currentPaletteName;
  let currentPaletteProperties;
  let currentPaletteIndexLowerBound = 0;
  let selectBlockIndex = 0; // currentPaletteBlocksIndexs ??
  ...
  if (
    ...
    !pos_in_box(
      bot.entity.position,
      build_cache.placement_origin.offset(-2, -100, -2),
      build_cache.placement_destination.offset(2, 100, 2),
    )
  ) {
  ...
  mapartBuild: while (build_cache.currentPalette < targetSch.palette.length) {
    ...
    if (!wheatherGetPalette) {
      ...
      currentPaletteBlocksState = []; //[0 ?��???][1 已�??�];
      currentPaletteName =
        targetSch.palette[sch_palette_order[build_cache.currentPalette]].Name;
      currentPaletteProperties =
        targetSch.palette[sch_palette_order[build_cache.currentPalette]]
          .Properties;
      //if(debug_enable)
      bot.logger(
        false,
        "INFO",
        `\x1b[32m?��??��?:\x1b[0m ${currentPaletteName} ${build_cache.currentPalette + 1}/${targetSch.palette.length}`,
      );
      ...
      for (let i = 0; i < IgnoreAirArray.length; i++) {
        await wait();
        if (
          targetSch.getBlockPIDByIndex(IgnoreAirArray[i]) ==
          sch_palette_order[build_cache.currentPalette]
        ) {
          //console.log(targetSch.getBlockByIndex(i))
    ...
    let currentBotPos = new Vec3(
      Math.round(bot.entity.position.x - 0.5),
      Math.round(bot.entity.position.y),
      Math.round(bot.entity.position.z - 0.5),
    );
    ...
    if (currentMapartServer > 0 && currentMapartServer != build_cache.server) {
      bot.logger(
        false,
        "WARN",
        `?��??�誤 ?��?:${currentMapartServer} ?��?:${build_cache.server}`,
      );
      await mcFallout.promiseTeleportServer(bot, build_cache.server, 15_000);
      // while (true) {
      //     //await sleep(100)
    ...
    if (
      ...
      console.log(
        `起始座標: ${build_cache.placement_origin.offset(-2, -100, -2)}`,
      );
      console.log(
        `結束座標: ${build_cache.placement_destination.offset(2, 100, 2)}`,
      );
      bot.chat(`/homes mapart`);
      await sleep(5000);
      console.log("座標修正完成");
    ...
    let selectBlockRelativePos = targetSch.vec3(
      currentPaletteBlocksIndexs[selectBlockIndex],
    );
    let selectBlockAbsolutePos = build_cache.placement_origin.plus(
      selectBlockRelativePos,
    );
    ...
    for (let cP_dz = -4; cP_dz <= 4; cP_dz++) {
      ...
      for (let cP_dx = -4; cP_dx <= 4; cP_dx++) {
        ...
        for (let cP_dy = 5; cP_dy >= -3; cP_dy--) {
          ...
          let currentPosPlace_d_Index = targetSch.index(
            dRelativePos.x,
            dRelativePos.y,
            dRelativePos.z,
          ); // real index
          ...
          if (
            targetSch.getBlockPIDByIndex(currentPosPlace_d_Index) !=
            sch_palette_order[build_cache.currentPalette]
          )
            continue; // ??? 不知?�幹??
          let dBlocksIndex = currentPaletteBlocksIndexs.indexOf(
            currentPosPlace_d_Index,
          ); // fast index
          //console.log(dBlocksIndex)
          if (
            currentPaletteBlocksState[dBlocksIndex] == 0 &&
            Block_In_CD.indexOf(dBlocksIndex) == -1
          ) {
            ...
            if (
              ...
              for (let idx = 9; idx <= 44; idx++) {
                ...
                if (
                  bot.inventory.slots[idx] != null &&
                  bot.inventory.slots[idx].name == currentPaletteName
                ) {
            ...
            if (hold == null || hold.name != currentPaletteName) {
              ...
              if (findMaterialSlot == -1) {
                ...
                // ?��??��?
                for (
                  let quantityrestock_i = 0;
                  quantityrestock_i < currentPaletteBlocksIndexs.length;
                  quantityrestock_i++
                ) {
                  //console.log(quantityrestock_index)
                  if (currentPaletteBlocksState[quantityrestock_i] == 1)
                    continue;
                ...
                needReStock.push({
                  ...
                  count: quantityrestock,
                  p: build_cache.currentPalette,
                });
                if (true) {
                  //console.log("Calculating Mutli Restock ")
                  //build_cache.currentPalette < targetSch.palette.length
                  for (
                    let crtRSC = build_cache.currentPalette + 1;
                    crtRSC < targetSch.palette.length;
                    crtRSC++
                  ) {
                    ...
                    let crtCanRSCount = 0;
                    let crtInv =
                      bot.inventory.countRange(
                        bot.inventory.inventoryStart,
                        bot.inventory.inventoryEnd,
                        mcData.itemsByName[targetSch.palette[realId].Name].id,
                        null,
                      ) ?? 0;
                    if (crtInv > 0) {
                      //?�裡?�要更精�??��???之�??��?
                    ...
                    needReStock.push({
                      name: targetSch.palette[realId].Name,
                      count: crtCanRSCount,
                      p: crtRSC,
                    });
                ...
                if (!mp) bot.logger(false, "DEBUG", "無");
                if (debug_enable) {
                  bot.logger(false, "DEBUG", `背包物品統計`);
                  for (const i in mp) {
                    bot.logger(
                      false,
                      "DEBUG",
                      `${i.slice(0, 16).toString().padEnd(16)} ${mp[i]}`,
                    );
                  }
                }
                ...
                console.log(
                  `${"物品名稱".padEnd(14, " ")}(${"id".padEnd(2)}) ${"數量".padEnd(2)} ${"組數".padEnd(1)}`,
                );
                for (let jj = 0; jj < needReStock.length; jj++) {
                  console.log(
                    `${needReStock[jj].name.slice(0, 16).padEnd(16, " ")}(${(needReStock[jj].p + 1).toString().padEnd(2)}) ${needReStock[jj].count.toString().padEnd(4)} ${Math.ceil(
                      needReStock[jj].count / 64,
                    )
                      .toString()
                      .padEnd(2)}`,
                  );
                ...
                if (cfg.materialsMode == "station") {
                  ...
                  await mcFallout.promiseTeleportServer(
                    bot,
                    stationConfig.stationServer,
                    15_000,
                  );
                  await sleep(2000);
                  ...
                  console.log(
                    `#materialsMode ${cfg.materialsMode} not implemented`,
                  );
                  return;
                }
              } else {
                if (debug_enable)
                  bot.logger(
                    false,
                    "INFO",
                    `\x1b[33m?��??��?欄\x1b[0m ${findMaterialSlot} <-> ?�`,
                  );
                await bot.simpleClick.leftMouse(44);
                await bot.simpleClick.leftMouse(findMaterialSlot);
                await bot.simpleClick.leftMouse(44);
            ...
            if (debug_enable)
              bot.logger(
                false,
                "DEBUG",
                `\x1b[32m?�置\x1b[0m ${currentPaletteName} (${build_cache.currentPalette}) ??a${dAbsolutePos} r${dRelativePos} dBI:${dBlocksIndex} r_i:${currentPosPlace_d_Index} ${targetSch.getBlockByIndex(currentPosPlace_d_Index).Name}`,
              );
            //??
    ...
    selectBlockIndex = -1;
    for (let layer = 0; selectBlockIndex == -1 && layer <= 64; layer++) {
      //60->8
      for (
        let fN_dx = 0 - layer;
        selectBlockIndex == -1 && fN_dx <= layer;
        fN_dx++
      ) {
        for (
          let fN_dz = 0 - layer;
          selectBlockIndex == -1 && fN_dz <= layer;
          fN_dz++
        ) {
          //???身�?符�??�block 一起�? ?��?設�?visit
          for (
            let fN_dy = layer;
            selectBlockIndex == -1 && fN_dy >= 0 - layer;
            fN_dy--
          ) {
            //console.log(`檢查 ${fN_dx} ${fN_dy} ${fN_dz}`);
            if (
              Math.abs(fN_dx) == layer ||
              Math.abs(fN_dy) == layer ||
              Math.abs(fN_dz) == layer
            ) {
              //?�檢?��?�?�� (不�?複檢?�中?��?)
              let fN_nowRpos = selectBlockRelativePos.offset(
                fN_dx,
                fN_dy,
                fN_dz,
              );
              ...
              let fN_nowPosRealIndex = targetSch.index(
                fN_nowRpos.x,
                fN_nowRpos.y,
                fN_nowRpos.z,
              );
              ...
              if (fnp != sch_palette_order[build_cache.currentPalette])
                continue;
              let fn_n_fastindex =
                currentPaletteBlocksIndexs.indexOf(fN_nowPosRealIndex);
              if (fn_n_fastindex == -1) continue;
              //?�裡?�能?�lag主�?
              if (
                currentPaletteBlocksState[fn_n_fastindex] == 0 &&
                Block_In_CD.indexOf(fn_n_fastindex) == -1
              ) {
    ...
    while (selectBlockIndex == -1) {
      let blockInCD = false;
      for (
        let lastFindNotFinish = currentPaletteIndexLowerBound;
        lastFindNotFinish < currentPaletteBlocksIndexs.length;
        lastFindNotFinish++
      ) {
        if (currentPaletteBlocksState[lastFindNotFinish] == 1)
          continue; //已�????��???
          ...
          let fNextRpos = targetSch.vec3(
            currentPaletteBlocksIndexs[lastFindNotFinish],
          ); //sch_blocksPos[lastFindNotFinish]    //?��?點座�??��?)
    ...
    //#實�? #EXP
    if (true) {
      //console.log(`\x1b[36mFIND NEXT?��? \x1b[33m${debug_endFNext - deubg_startFNext}\x1b[0m ms`);
      build_cache.debug.findNextTotalCounter +=
        debug_endFNext - deubg_startFNext;
    }
  ...
  function updateVisited(oldBlock, newBlock) {
    ...
    if (
      !pos_in_box(
        updatePos,
        build_cache.placement_origin,
        build_cache.placement_destination,
      )
    )
      return;
    //if (!pos_in_box(updatePos, placement_origin, placement_end)) return;
    let r_update_pos = updatePos.minus(build_cache.placement_origin); //轉為?��??�內??
    let targetIndex = targetSch.index(
      r_update_pos.x,
      r_update_pos.y,
      r_update_pos.z,
    );
    ...
    if (
      currentPaletteName == newBlock.name &&
      currentPaletteName == oldBlock.name
    ) {
      if (currentPaletteBlocksState[qindex] == 0) {
        currentPaletteBlocksState[qindex] = 1;
        build_cache.placedBlock++;
      }
      //console.log("?�置認�?"+r_update_pos,qindex);
    } else {
      //blocksvisit[targetIndex]=-2;    //error
      // console.log("?�測?�錯誤�??�置"+updatePos);
    }
  }
  // #DEBug
  if (true) {
    console.log(`?��?補�?次數 ${build_cache.debug.restock_count} 次`);
    console.log(`?��?補�?總耗�?: ${build_cache.debug.restock_takeTime} ms`);
    console.log(`FN總耗�?: ${build_cache.debug.findNextTotalCounter} ms`);
  }
  return "finish";
}
/**
...
async function model_building_build(task, bot, cfg, project) {
  ...
  let layerPaletteCountByIndex = {}; //?�PID??
  // {
  //     '46': 1,
  //     '51': 1,
  //     '53': 2,
  ...
  //     '67': [ 8536 ]
  // }
  let currentPaletteBlocksState = []; // [0 ?��???][1 已�??�][2 ?�?�錯誤];
  let blockInCD = [];
  let removeTimerID = [];
  let sch_palette_order = [];
  let needStore = [];
  ...
  buildLoop: while (build_cache.currentLayer < build_cache.totalLayer) {
    // targetSch.palette.length
    await wait();
    if (stop) return;
    if (pause) {
      await sleep(500);
    ...
    if (!wheatherGetLayerPalette) {
      if (debug_enable)
        console.log(`計�? �?${build_cache.currentLayer} �??��?`);
      startIndex =
        build_cache.currentLayer *
        targetSch.Metadata.EnclosingSize.x *
        targetSch.Metadata.EnclosingSize.z;
      endIndex =
        (build_cache.currentLayer + 1) *
          targetSch.Metadata.EnclosingSize.x *
          targetSch.Metadata.EnclosingSize.z -
        1;
      ...
      for (let i = 0; i < targetSch.palette.length; i++) {
        if (layerPaletteCountByIndex[i] == 0) {
          ...
          console.log("跳�?" + targetSch.palette[i].Name);
          delete layerPaletteCountByIndex[i];
          delete layerPaletteBlocksByIndex[i];
      ...
      //console.log(sch_palette_order)
      sch_palette_order = sch_palette_order.map((item) =>
        parseInt(item.index, 10),
      );
      //console.log(sch_palette_order)
      //計�??��??��? ?��??�優????Properties ???��?
      console.log(
        `id ${startIndex} - ${endIndex} ??${sch_palette_order.length} ?�`,
      );
    ...
    if (!wheatherGetPalette) {
      ...
      bot.logger(
        false,
        "INFO",
        `\x1b[32m?��??��?:\x1b[0m ${targetSch.palette[sch_palette_order[build_cache.currentPalette]].Name} ${build_cache.currentPalette + 1}/${Object.keys(layerPaletteCountByIndex).length}`,
      );
      currentPaletteBlocksState = new Array(
        layerPaletteCountByIndex[sch_palette_order[build_cache.currentPalette]],
      ).fill(0);
      // [0 ?��???][1 已�??�];
      currentPalette =
        targetSch.palette[sch_palette_order[build_cache.currentPalette]];
      // if (debug_enable) console.log("?�?�表", currentPaletteBlocksState)
      // if (debug_enable) console.log("實�?Index", layerPaletteBlocksByIndex[sch_palette_order[build_cache.currentPalette]])
    ...
    let selectBlockRealIndex =
      layerPaletteBlocksByIndex[sch_palette_order[build_cache.currentPalette]][
        selectBlockIndex
      ];
    ...
    for (let cP_dz = -4; cP_dz <= 4; cP_dz++) {
      ...
      for (let cP_dx = -4; cP_dx <= 4; cP_dx++) {
        ...
        let dAbsolutePos = selectBlockAbsolutePos.offset(cP_dx, cP_dy, cP_dz);
        if (
          !pos_in_box(dRelativePos, build_cache.origin, build_cache.destination)
        )
          continue;
        let blockCenterPos = dAbsolutePos.plus(new Vec3(0.5, 0.5, 0.5));
        let distanceEyesToCenter = botEyePosition.distanceTo(blockCenterPos);
        ...
        if (
          ...
          continue;
        let dBlocksIndex = layerPaletteBlocksByIndex[
          sch_palette_order[build_cache.currentPalette]
        ].indexOf(currentPosPlace_d_Index); // fast index
        if (
          blockInCD.indexOf(dBlocksIndex) == -1 &&
          checkBlock(bot.blockAt(dAbsolutePos), currentPalette) == 1
        ) {
          //bot.blockAt(selectBlockAbsolutePos)?.name == currentPalette.Name) {
          //console.log(currentPalette.Name, bot.blockAt(selectBlockAbsolutePos)?.name)
          currentPaletteBlocksState[dBlocksIndex] = 1; //[2 ?�?�錯誤]
        } else {
          ...
          if (
            ...
            if (bot.blockAt(dAbsolutePos) == null) continue;
            //?�新檢查該�?�?
            let checkBlockID = checkBlock(
              bot.blockAt(dAbsolutePos),
              currentPalette,
            );
            ...
            if (checkBlockID == 1) {
              ...
              if (
                ...
                for (let idx = 9; idx <= 44; idx++) {
                  ...
                  if (
                    bot.inventory.slots[idx] != null &&
                    bot.inventory.slots[idx].name == currentPalette.Name
                  ) {
                    findMaterialSlot = idx;
                    //console.log("?��?slot"+idx);
                    break;
              ...
              if (hold == null || hold.name != currentPalette.Name) {
                ...
                if (findMaterialSlot == -1) {
                  ...
                  for (
                    let quantityrestock_i = 0;
                    quantityrestock_i < currentPaletteBlocksState.length;
                    quantityrestock_i++
                  ) {
                    //console.log(quantityrestock_index)
                    if (currentPaletteBlocksState[quantityrestock_i] == 1)
                  ...
                  let quantityrestock_useSlot = Math.ceil(
                    quantityrestock /
                      mcData.itemsByName[currentPalette.Name].stackSize,
                  );
                  emptySlotCount -= quantityrestock_useSlot;
                  needReStock.push({
                  ...
                  if (true) {
                    ...
                    for (
                      let crtRSC = build_cache.currentPalette + 1;
                      crtRSC < sch_palette_order.length;
                      crtRSC++
                    ) {
                      if (emptySlotCount < 1) break;
                      let realId = sch_palette_order[crtRSC];
              ...
              if (debug_enable) {
                console.log(
                  `\x1b[32m?�置\x1b[0m ${currentPalette.Name} (${build_cache.currentPalette}) ??${dAbsolutePos}`,
                );
              }
            }
          }
        ...
        for (
          ...
          if (
            ...
            let fn_n_fastindex =
              layerPaletteBlocksByIndex[
                sch_palette_order[build_cache.currentPalette]
              ].indexOf(fN_nowPosRealIndex); //???
            if (fn_n_fastindex == -1) continue;
            //?�裡?�能?�lag主�?
            if (
              currentPaletteBlocksState[fn_n_fastindex] == 0 &&
              blockInCD.indexOf(fn_n_fastindex) == -1
            ) {
              if (bot.blockAt(fn_nowApos) == null) {
                continue;
              }
...
async function model_redstone_build(task, bot, cfg, project) {
...
function checkBlock(block, TargetPalette) {
  ...
  for (let i in BLOCK_EQUIVALENT_LIST) {
...
async function save_cache(bot_id, cache) {
  await fsp.writeFile(
    `${process.cwd()}/config/${bot_id}/build_cache.json`,
