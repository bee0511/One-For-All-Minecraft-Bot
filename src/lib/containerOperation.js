...
const containerOperation = {
   ...
   * @param {number} reserve - 預留空間
     * @returns {Promise<number>} - 未完成數量(如inv還夠)
   */
  async withdraw(bot, container, id, count, dontlog, reserve) {
    ...
    let targetItemInContainerCount = container.countRange(
      0,
    ...
    try {
      ...
      if (!dontlog)
        console.log(`\x1b[31m取出 x${count} \x1b[36m${targerItem.name}\x1b[0m`);
    } catch (e) {
  ...
  async deposit(bot, container, id, count, dontlog) {
    ...
    if (dontlog == null || dontlog == undefined) dontlog = false;
    //console.log(container)
    let targetItemInInvCount = container.countRange(
      container.inventoryStart,
    ...
    let maxCanDeposit =
      container.inventoryStart * targerItem.stackSize -
      targetItemInContainerCount;
    ...
    try {
      ...
      if (!dontlog)
        console.log(`\x1b[32m放入 x${count} \x1b[36m${targerItem.name}\x1b[0m`);
    } catch (e) {
  ...
  async throw_slot(bot, slot) {
    ...
    if (bot.inventory.slots[slot] != null) {
      bot.logger(
        ...
        "INFO",
        process.argv[2],
        `丟棄 slot: ${slot} - ${bot.inventory.slots[slot].name} x${bot.inventory.slots[slot].count}`,
      );
