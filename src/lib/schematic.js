...
const schematic = {
  ...
  loadFromLitematic: async function (path) {
    ...
    let tglt = new sch(
      raw_Litematic.Metadata.Name,
      raw_Litematic.Metadata.EnclosingSize.x,
      raw_Litematic.Metadata.EnclosingSize.y,
      raw_Litematic.Metadata.EnclosingSize.z,
    );
    tglt.Metadata = raw_Litematic.Metadata;
    tglt.Metadata.EnclosingSize = new Vec3(
      raw_Litematic.Metadata.EnclosingSize.x,
      raw_Litematic.Metadata.EnclosingSize.y,
      raw_Litematic.Metadata.EnclosingSize.z,
    );
    tglt.Metadata.TimeCreated = Date.now();
    tglt.Metadata.TimeModified = Date.now();
    if (tglt.Metadata.RegionCount != 1)
      throw new Error("Litematic Multi Regions Not Support");
    ...
    const arraySizeIn = Math.ceil(
      (bitsPerEntry *
        tglt.Metadata.EnclosingSize.x *
        tglt.Metadata.EnclosingSize.y *
        tglt.Metadata.EnclosingSize.z) /
        64,
    );
    tglt.LitematicaBitArray = new LitematicaBitArray(
      bitsPerEntry,
      arraySizeIn,
      firstRegion.BlockStates,
    );
    return tglt;
  ...
  loadFromNbt: async function (name = "default", path) {
    ...
    // 下面?????
    if (airindex == -1) {
      for (let b_index of raw_nbtFile.blocks) {
        await wait();
        //console.log("pos:",b_index.pos[0], b_index.pos[1], b_index.pos[2],"index:",tglt.index(b_index.pos[0], b_index.pos[1], b_index.pos[2]))
        tglt.LitematicaBitArray.setAt(
          tglt.index(b_index.pos[0], b_index.pos[1], b_index.pos[2]),
          b_index.state + 1,
        );
      ...
      tglt.palette.unshift({ Name: air });
      for (const b_index of raw_nbtFile.blocks) {
        await wait();
...
class sch {
  ...
  getPaletteIndex(blockState) {
    let index = this.palette.findIndex((item) => {
      return (
        item.Name === blockState.Name &&
        JSON.stringify(item.p) === JSON.stringify(blockState.p)
      );
    });
    if (index == -1) {
  ...
  index(x, y, z) {
    return (
      y * this.Metadata.EnclosingSize.x * this.Metadata.EnclosingSize.z +
      z * this.Metadata.EnclosingSize.x +
      x
    );
  }
  vec3(id) {
    let y = Math.floor(
      id / (this.Metadata.EnclosingSize.x * this.Metadata.EnclosingSize.z),
    );
    let z = Math.floor(
      (id % (this.Metadata.EnclosingSize.x * this.Metadata.EnclosingSize.z)) /
        this.Metadata.EnclosingSize.x,
    );
  ...
  setBlock(x, y, z, block) {
    if (outOfRange(this.Metadata.EnclosingSize, x, y, z)) {
      throw new Error(
        `SetBlock out of range : enclosingSize: ${this.Metadata.EnclosingSize},Pos: ${x}, ${y}, ${z}`,
      );
    }
    ...
    this.LitematicaBitArray.setBlock(
      this.index(x, y, z),
      this.getPaletteIndex(block),
    );
  ...
  getBlock(x, y, z) {
    const paletteIndex = Number(
      this.LitematicaBitArray.getAt(this.index(x, y, z)),
    );
    return this.palette[paletteIndex];
    // Implementation of GetBlock
  }
  getBlockByIndex(index) {
    const paletteIndex = Number(this.LitematicaBitArray.getAt(index));
    return this.palette[paletteIndex];
    // Implementation of GetBlock
  }
...
class LitematicaBitArray {
  /**
   *
   * @param {number} bitsPerEntryIn
   * @param {number} arraySizeIn
  ...
  setBlock(index, value) {
    if (value > this.bitsPerEntry) {
      let newLitematicBitArray = this.resize(
        this.bitsPerEntry + 1,
        this.arraySize,
      );
  ...
  setAt(index, value) {
    ...
    this.longArray[startArrIndex] =
      (this.longArray[startArrIndex] &
        ~(this.maxEntryValue << startBitOffset)) |
      ((BigInt(value) & this.maxEntryValue) << startBitOffset);

    if (startArrIndex != endArrIndex) {
      let endOffset = BigInt(64) - startBitOffset;
      let j1 = BigInt(this.bitsPerEntry) - endOffset;
      this.longArray[endArrIndex] =
        (unSignedRightShift(this.longArray[endArrIndex], j1) << j1) |
        unSignedRightShift(BigInt(value) & this.maxEntryValue, endOffset);
    }
  }
  getBlock(index) {
  ...
  getAt(index) {
    ...
    if (startArrIndex == endArrIndex) {
      return (
        unSignedRightShift(this.longArray[startArrIndex], startBitOffset) &
        this.maxEntryValue
      );
  ...
  resize(bits, EntrySize) {
    let newLitematicBitArray = new LitematicaBitArray(bits, EntrySize, null);
...
function litematicParsePalette(region) {
  let palette = [];
  for (let np in region.BlockStatePalette) {
...
function unSignedRightShift(bigint_num, bigint_shiftbit) {
  const shift = BigInt(bigint_shiftbit);
  const value = BigInt(bigint_num);
  if (value >= 0n) return value >> shift;
  let mask = 0n;
  let tss = 64 - Number(bigint_shiftbit);
  for (let i = 0; i < tss; i++) {
    mask = (mask << 1n) + 1n;
  }
